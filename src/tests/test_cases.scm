...
(test-case '(let* ([x 1])
              (let* ([x (+ x 1)] [y (+ x 1)])
                y)) "3" "Let*")
(test-case '(or #t #f) "1" "Or (first expr is true)")
(test-case '(or #t #t) "1" "Or (both true)")
(test-case '(or #f #f) "0" "Or (both false)")
(test-case '(or #f #t) "1" "Or (second one is true)")
(test-case '(and #t #f) "0" "And (first expr is true)")
(test-case '(and #t #t) "1" "And (both true)")
(test-case '(and #f #f) "0" "And (both false)")
(test-case '(and #f #t) "0" "And (second one is true)")
(test-case '(letrec ([x 5]) x) "5" "Letrec (simple)")
...
(test-case '(funcall (labels ((f0 (code (x y) () (+ x y))))
                    (closure f0)) 2 3) "5" "Closure (simple v2)")
(test-case '(labels ((f0 (code (x y) () (+ x y))))
                    (+ (funcall (closure f0) 2 3) (funcall (closure f0) 2 3))) "10" "Closure (call same closure twice)")
(test-case '(labels ((f0 (code (x y) () (+ x y)))
                      (f1 (code (y) () (funcall (closure f0) y 1))))
                    (funcall (closure f1) 4)) "5" "Closure (f1 calls f0)")
(test-case '(labels ((f0 (code (x y) () (let ([i 5] [j 6]) (+ x (- i (* j y)))))))
                      (funcall (closure f0) 2 3)) "-11" "Closure (with let)")
(test-case '(let ([f (lambda () 12)]) (funcall f)) "12" "Lambda (simple)")
(test-case '(funcall (lambda (x y) (+ x y)) 2 3) "5" "Lambda (simple v2)")
(test-case '(let ([f (lambda () 12)]) (+ (funcall f) (funcall f))) "24" "Lambda (called twice)")
(test-case '(let ([g (lambda () (+ 2 3))]) (* (funcall g) (funcall g))) "25" "Lambda (called twice v2)")
(test-case '(let ([f (lambda ()
                      (let ([g (lambda () (+ 2 3))])
                        (* (funcall g) (funcall g))))]) (funcall f)) "25" "Lambda (nested)")
(test-case '(let ([f (if (boolean? (lambda () 12))
                          (lambda () 13)
                          (lambda () 14))])
                  (funcall f)) "14" "Lambda (with if statement)")
(test-case '(let ([f (lambda (x)
                        (let ([g (lambda (x y) (+ x y))])
                              (funcall g x 100)))])
                  (funcall f 1000)) "1100" "Lambda (parameter passing)")
(test-case '(let ([f (lambda (g) (funcall g 2 13))])
                  (funcall f (lambda (n m) (* n m)))) "26" "Lambda (parameter passing 2)")
(test-case '(let ([f (lambda (g) (+ (funcall g 10) (funcall g 100)))])
                 (funcall f (lambda (x) (* x x)))) "10100" "Lambda (parameter passing 3)")
(test-case '(let ([f (lambda (f n m)
                        (if (zero? n)
                            m
                            (funcall f f (sub1 n) (* n m))))])
                  (funcall f f 5 1)) "120" "Lambda (parameter passing 4)")
(test-case '(let ([f (lambda (f n)
                        (if (zero? n)
                            1
                            (* n (funcall f f (sub1 n)))))])
                  (funcall f f 5)) "120" "Lambda (parameter passing 5)")
(test-case '(letrec ([f (lambda (f n)
                        (if (zero? n)
                            1
                            (* n (funcall f f (sub1 n)))))])
                  (funcall f f 5)) "120" "Lambda (parameter passing with letrec)")
(test-case '(let ([n 12])
                  (let ([f (lambda () n)])
                        (funcall f))) "12" "Lambda (basic closure)")
(test-case '(let ([n 12])
                  (let ([f (lambda (m) (+ n m))])
                        (funcall f 100))) "112" "Lambda (closure)")
(test-case '(funcall
              (funcall (let ((x 5))
                              (lambda (y)
                                      (lambda ()
                                              (+ x y))))
                2)) "7" "Lambda (nested closure)")
(test-case '(let ([f (lambda (f n m)
                                (if (zero? n)
                                    m
                                    (funcall f (sub1 n) (* n m))))])
                  (let ([g (lambda (g n m) (funcall f (lambda (n m) (funcall g g n m)) n m))])
                        (funcall g g 5 1))) "120" "Lambda (big nested)")
(test-case '(let ([f (lambda (f n)
                              (if (zero? n)
                                  1
                                  (* n (funcall f (sub1 n)))))])
                  (let ([g (lambda (g n) (funcall f (lambda (n) (funcall g g n)) n))])
                        (funcall g g 5))) "120" "Lambda (big nested v2)")
(test-case '(letrec ([is-even? (lambda (n)
                       (or (zero? n)
                           (funcall is-odd? (sub1 n))))]
                      [is-odd? (lambda (n)
                                  (and (not (zero? n))
                                        (funcall is-even? (sub1 n))))])
                    (funcall is-odd? 11)) "1" "Lambda (with letrec)")